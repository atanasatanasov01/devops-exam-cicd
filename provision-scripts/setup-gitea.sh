...
# Wait until ready
while true; do 
  echo 'Trying to connect to http://192.168.150.202:3000 ...'; 
  if [ $(curl -s -o /dev/null -w "%{http_code}" http://192.168.150.202:3000) == "200" ]; then 
    echo '... connected.'; 
    break; 
  else 
...
cd /tmp/exam && git push -o repo.private=false http://vagrant:vagrant@192.168.150.202:3000/vagrant/exam

# Add a Webhook
curl -X 'POST' 'http://192.168.150.202:3000/api/v1/repos/vagrant/exam/hooks' \
  ...
  "config": {
    "content_type": "json",
    "url": "http://192.168.150.201:8080/gitea-webhook/post",
    "http_method": "post"
  },
  "events": [
    "push"
